let points =[[-4,7],[-2,8],[0,8],[0,9],[3,9],[6,7],[8,4],[9,5],[9,4],[8,3],[9,3], [9, 2], [8,2], 
[5, -1],[3,-1],[3,-2],[-1,-2],[1,-1],[-2,-1],[-3,0],[-5,2],[-6,0],[-9,-2],[-9,1],[-7,3],[-9,5],
[-9,9], [-6,6], [-5, 4],[-3,6],[-4,7]]; //list資料，
...
function setup() {  //設定魚魚物件倉庫內的資料
  ...
  for(var j=0;j<15;j=j+1)
  {
    ...
    fishhh.push(fish)  //把ball物件放入到balls物件倉庫(陣列)中
  }
...
function draw() {  //每秒會執行60次次
  ...
  for(let fish of fishhh){  //針對陣列變數，取出陣列內一個一個的物件
    ...
    for(let bullet of bullets){
      if(fish.isBallInRanger(bullet.p.x,bullet.p.y))  //判斷ball與bullet有沒有碰觸
      {
        score = score - 1     //分數扣一
        elephant_sound.play()
